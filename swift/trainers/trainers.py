...
class Seq2SeqTrainer(PushToMsHubMixin, SwiftMixin, HfSeq2SeqTrainer):
    ...
    @staticmethod
    def compute_margin_loss(pos_logits: torch.Tensor, neg_logits: torch.Tensor, labels: torch.Tensor, margin: float = 0.2) -> torch.Tensor:
        ...
        pos_scores = pos_logits[..., :-1, :][masks].gather(1, shift_labels.unsqueeze(1)).squeeze(1)
        neg_scores = neg_logits[..., :-1, :][masks].gather(1, shift_labels.unsqueeze(1)).squeeze(1)
    ...
    def compute_loss(self, model, inputs, return_outputs=None):
        ...
        margin_loss = self.compute_margin_loss(pos_outputs.logits, neg_outputs.logits, pos_labels)
